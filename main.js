...
const createProjectCards = (arr) => {
    ...
    for (let i = 0; i < arr.length; i++) {
        if (arr[i].available) {
            ...
            domString +=        `<button class="github-Url-button btn btn-dark btn-lg ml-2"><a class="text-white" href=${arr[i].githubUrl} target="_blank">View on GitHub</a></button>`;
            domString +=     `</div>`;
            domString += `</div>`;
            domString += `</div>`;  
            domString += `</div>`;  
